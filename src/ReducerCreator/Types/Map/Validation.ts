...
export const checkValidationOfMapAction = (action: any, dataObjectKeyName: string) => {
    if (['Set', 'Add', 'Add/Ignore', 'Add/Replace', 'Add/Merge', 'Replace', 'Merge']
        .find(command => action.type.startsWith(command))) {
        if (!onlyOneExists(action.key != undefined && action.value != undefined, action.keyValue, action.data != undefined, action.map != undefined))
        ...
        if (action.key != undefined) {
            if (typeof action.key != 'string' && typeof action.key != 'number')
                return 'For `redux_map` type, action.key must be string or number.';
            if (action.value == undefined)
                return 'For `redux_map` type, action.key must be beside of action.value.';
        }
        else if (action.keyValue) {
            if (typeof action.keyValue.key != 'string' && typeof action.keyValue.key != 'number')
                return 'For `redux_map` type, action.keyValue.key must be string or number.';
            if (action.keyValue.value == undefined)
                return 'For `redux_map` type, action.keyValue.key must be beside of action.keyValue.value.';
            if (Array.isArray(action.keyValue)) {
                if (action.keyValue.find((kv: any) => !action.keyValue[kv] ||
                    ...
                    return 'For `redux_map` type, action.keyValue as array must be of ' +
            ...
            if (Array.isArray(action.data)) {
                if (action.data.find((d: any) => d[dataObjectKeyName] == undefined))
                    return 'For `redux_map` type, action.data can be array of data,' +
                ...
                return 'For `redux_map` type, action.data can be object,' +
                    'but must property with name <dataObjectKeyName>.\n' +
                    '* <dataObjectKeyName> is defined when creating the reducer.' +
                    '* <dataObjectKeyName> value is `id` by default'
        }
        else if (action.map) {
            if (typeof action.map != 'object')
                return 'For `redux_map` type, action.map must be an object. (each property is key-value-pair in map';
        }
    }
    else if (action.type.startsWith('Remove')) {
        if (action.key == undefined)
            return 'For `redux_map` to remove an item, actions should have key';

        if ((typeof action.key != 'string' && typeof action.key != 'number' &&
            (!Array.isArray(action.key) ||
                action.find((a: any) => a != 'string' && typeof a != 'number'))))
            return 'For `redux_map` type, action.key as array only must be of type one of below :\n' +
                '   → string\n' +
                '   → number\n' +
