...
export const getMapHelpReaction = (actionTypes: MapActionTypes[], name: string, dataObjectKeyName: string) => {
    return (state: any) => {
        const availableActions = actionTypes.map(at => {
            switch (at) {
                case 'Set':
                    return `\x1b[33m→ ${at}_${name} \x1b[37m ` + 'Creates new map and pushes `key-value-data` into it.\n' +
                        '   * `key-value-data` is described at the end.';
                case 'Add/Replace':
                    return `\x1b[33m→ ${at}_${name} \x1b[37m ` + 'Adds an `key-value-data` to the related redux map,\n' +
                ...
                case 'Add/Merge':
                    return `\x1b[33m→ ${at}_${name} \x1b[37m ` + 'Adds an `key-value-data` to the related redux map.\n' +
                        ...
                        '   * `key-value-data` is described at the end.';
                case 'Replace':
                    return `\x1b[33m→ ${at}_${name} \x1b[37m ` + 'Replaces an `key-value-data` to the same-key-data(s) the related redux map.\n';
                case 'Merge':
                    return `\x1b[33m→ ${at}_${name} \x1b[37m ` + 'Merges an `key-value-data` to the same-key-data(s) the related redux map.\n';
                case 'Remove':
